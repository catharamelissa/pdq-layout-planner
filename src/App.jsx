...
const BRAND_COLORS = {
  "Bath & Body": {hdr:"#3d6b2a",hdrTxt:"#fff",bucketHdr:"#4e7f36",bucketTxt:"#fff",light:"#eaf5e2",border:"#5a9440"},
  "Kids Bath & Body":{hdr:"#0e6b6b",hdrTxt:"#fff",bucketHdr:"#1a8080",bucketTxt:"#fff",light:"#ddf1f1",border:"#1a9999"},
  "Baby Bath & Body":{hdr:"#4a3d8a",hdrTxt:"#fff",bucketHdr:"#5c4ea8",bucketTxt:"#fff",light:"#f0edf9",border:"#7060c0"},
};
const PALETTE = ["#e63946","#f4a261","#2a9d8f","#4361ee","#9b2335","#e9c46a","#7b2d8b","#43aa8b","#f79d65","#264653","#6a994e","#4895ef"];
const GRID = 0.25, WALL = 0.4375;
...
function shelfWt(pls,prods) {
  return pls.filter(p=>!p.invalid).reduce((s,pl)=>{
    const p=prods.find(x=>x.id===pl.pid); return s+(p?p.wt:0)*(pl.layers||1);
  },0);
}
...
function buildColorMap(shelf,prods) {
  const q=shelf.qtys||{};
  const ap=prods.filter(p=>q[p.id]&&q[p.id].asst>0).sort((a,b)=>((q[a.id]||{}).order||999)-((q[b.id]||{}).order||999));
  const map={};let i=0;
    ...
    arr.forEach((e,ai)=>{
      ...
      for(let k=0;k<qty;k++){
        ...
        for(let m=0;m<matching.length;m++){
          const c=matching[m];
          if(c.nextY+p.w<=tray.w+0.01){pls.push({id:"pl_"+pls.length+"_"+Date.now(),pid:p.id,ai,layers,x:c.x,y:c.nextY,invalid:false});c.nextY+=p.w+gap;placed=true;break;}
        }
        if(placed) continue;
        let x=0;for(let j=0;j<cols.length;j++)x=Math.max(x,cols[j].x+cols[j].w+gap);
    ...
    arr.forEach((e,ai)=>{
      ...
      for(let k=0;k<qty;k++){
        ...
        for(let c=0;c<200&&!placed;c++){
          const tx=c*(p.l+gap);if(tx+p.l>tray.l+0.01)break;
          for(let r=0;r<200&&!placed;r++){
            const ty=r*(p.w+gap);if(ty+p.w>tray.w+0.01)break;
...
function alignBottomRight(pls,prods,tray) {
  const valid=pls.filter(p=>!p.invalid);if(!valid.length)return pls;
  let x1=-Infinity,y1=-Infinity;
  valid.forEach(pl=>{const p=prods.find(x=>x.id===pl.pid);if(!p)return;x1=Math.max(x1,pl.x+p.l);y1=Math.max(y1,pl.y+p.w);});
  if(!isFinite(x1)||!isFinite(y1))return pls;
  const ox=Math.max(0,tray.l-x1);
  const oy=Math.max(0,tray.w-y1);
  if(!ox&&!oy)return pls;
  return pls.map(pl=>pl.invalid?pl:{...pl,x:pl.x+ox,y:pl.y+oy});
}
...
function Canvas({shelf,allProds,colorMap,selectedId,onSelect,onMove,snapEnabled}) {
  ...
  useEffect(()=>{
    const mv=e=>{
      const d=dragRef.current;if(!d||!innerRef.current)return;
      const p=allProds.find(x=>x.id===d.pid);if(!p)return;
  ...
  return (
    <Container shape={shelf.shape} outer={outer} tray={tray} scale={scale} innerRef={innerRef} showLabels={true}>
      {shelf.pls.filter(pl=>!pl.invalid).map(pl=>{
        const p=allProds.find(x=>x.id===pl.pid);if(!p)return null;
        const col=colorMap[pl.pid+"|"+pl.ai]||"#888";
        const fs=Math.max(9,Math.min(16,Math.min(p.l,p.w)*scale/2.5));
        return <Item key={pl.id} pl={pl} prod={p} tray={tray} color={col} selected={pl.id===selectedId} fontSize={fs}
          onMouseDown={e=>{e.preventDefault();onSelect(pl.id);const r=innerRef.current.getBoundingClientRect();dragRef.current={id:pl.id,pid:pl.pid,ox:(e.clientX-r.left)/W*tray.l-pl.x,oy:(e.clientY-r.top)/H*tray.w-pl.y};}}/>;
      })}
    </Container>
  );
}
...
export default function App() {
  const [shelves,setShelves]=useState([makeShelf(1)]);
  const [savedShelves,setSavedShelves]=useState([]);
  ...
  const resetTimer=useRef(null),wrapRef=useRef(null);
  ...
  useEffect(()=>{
    ...
    s.onload=()=>setPdfReady(true);document.head.appendChild(s);
  },[]);
  ...
  /* ── Live capacity meter — total footprint area of entered quantities vs. usable area ── */
  const trayArea = useMemo(() => {
    return active.shape === "oval" ? Math.PI * (tray.l / 2) * (tray.w / 2) : tray.l * tray.w;
  }, [active.shape, tray.l, tray.w]);

  const usedArea = useMemo(() => {
    let area = 0;
    allProds.forEach(p => {
      const q = qtys[p.id];
      if (!q || !q.eachArr) return;
      q.eachArr.forEach(e => { area += ((e && e.qty) || 0) * p.l * p.w; });
    });
    return area;
  }, [allProds, qtys]);

  const capPct = trayArea > 0 ? (usedArea / trayArea) * 100 : 0;
  const capBarPct = Math.min(100, capPct);
  const capColor = capPct > 100 ? "#e24b4a" : capPct > 85 ? "#ef9f27" : "#1d9e75";
  ...
  const setEntry=useCallback((pid,ai,field,val)=>{
    ...
    setShelves(prev=>prev.map((s,i)=>{
      ...
      const newPls=field==="layers"
        ?s.pls.map(pl=>pl.pid===pid&&pl.ai===ai?Object.assign({},pl,{layers:n}):pl)
        :s.pls;
      return Object.assign({},s,{qtys:q,pls:newPls});
    }));
  },[idx]);
  ...
  const handleShelfDrop=useCallback(tid=>{
    ...
    setShelves(prev=>{
      ...
      setActiveIdx(re.findIndex(s=>s.id===active.id));return re;
    });
  ...
  const autoArr=useCallback(()=>{
    ...
    const final=sh.shape==="rect"?alignBottomRight(res.pls,allProds,t):res.pls;
  ...
  const exportPDF=()=>{
    const J=window.jspdf;if(!J){alert("PDF library still loading, try again.");return;}
    ...
    savedShelves.forEach((sh,si)=>{
      if(si>0)doc.addPage();
      const tray2=sh.tray||{l:12,w:8};
      const od=sh.outer||{l:round4(tray2.l+WALL),w:round4(tray2.w+WALL)};
      ...
      doc.text(`${sh.shape==="oval"?"Basket":"Tray"} · ${od.l}"x${od.w}" outer / ${tray2.l}"x${tray2.w}" inner · Saved: ${sh.savedAt}`,M,M+11);
      doc.setTextColor(0);
      const vX=M,vY=M+17,vmW=130,vmH=PH-vY-M-12;
      const sc=Math.min(vmW/od.l,vmH/od.w);
      const ow=od.l*sc,oh=od.w*sc,iw=tray2.l*sc,ih=tray2.w*sc;
      ...
      if(sh.shape!=="oval"){doc.setDrawColor(225,225,225);doc.setLineWidth(0.1);for(let ci=1;ci<Math.floor(tray2.l);ci++)doc.line(ix+ci*sc,iy,ix+ci*sc,iy+ih);for(let ri=1;ri<Math.floor(tray2.w);ri++)doc.line(ix,iy+ri*sc,ix+iw,iy+ri*sc);}
      vp.forEach(pl=>{
        const p=allProds.find(x=>x.id===pl.pid);if(!p)return;
      ...
      doc.text(`${od.l}" outer · ${tray2.l}" inner`,vX+ow/2,vY+oh+4,{align:"center"});
      ...
      [["POSITIONS",vp.length],["UNITS",units],["WEIGHT",(sh.wt||0).toFixed(2)+" lbs"],["LIMIT",sh.wtLimitI||"–"]].forEach(([lbl,val],i)=>{
        const sx=vX+i*33;doc.setFontSize(6);doc.setFont("helvetica","normal");doc.setTextColor(120);doc.text(lbl,sx,stripY);
        doc.setFontSize(11);doc.setFont("helvetica","bold");doc.setTextColor(0);doc.text(String(val),sx,stripY+5.5);
      });
      const tX=M+vmW+8,tW=PW-tX-M;let ty=M+17;
      const grp={};vp.forEach(pl=>{const k=pl.pid+"|"+pl.ai;if(!grp[k])grp[k]={c:0,ly:pl.layers||1,pid:pl.pid,ai:pl.ai};grp[k].c++;});
      ...
      let cx=tX+1.5;cHdrs.forEach((h,i)=>{doc.text(h,i===0?cx:cx+cW[i]-1.5,ty+3.8,{align:i===0?"left":"right"});cx+=cW[i];});
      ty+=5.5;
      Object.keys(grp).forEach((k,ri)=>{
        ...
        doc.setFontSize(6.5);doc.setFont("helvetica","normal");doc.setTextColor(0);cx=tX+4;
        [p.name,al(g2.ai),String(g2.c),String(g2.ly),String(g2.c*g2.ly),p.wt?p.wt.toFixed(2):"–",p.wt?(p.wt*g2.c*g2.ly).toFixed(2):"–"].forEach((v,vi)=>{
          doc.text(v,vi===0?cx:cx+cW[vi]-1.5,ty+3.8,{align:vi===0?"left":"right",maxWidth:cW[vi]-1.5});cx+=cW[vi];
        });
        doc.setDrawColor(230,230,230);doc.setLineWidth(0.1);doc.line(tX,ty+5.5,tX+tW,ty+5.5);ty+=5.5;
      });
    });

    doc.addPage();
    doc.setFont("helvetica","bold");doc.setFontSize(13);doc.setTextColor(0);doc.text("All Shelves — Summary",M,M+5);
    ...
    [["Shelves",savedShelves.length],["Positions",totPos],["Units",totUnits],["Total Weight",totalData.grandWt.toFixed(2)+" lbs"]].forEach(([lbl,val],i)=>{
      const bx=M+i*60,bY=M+12;doc.setFillColor(245,245,245);doc.roundedRect(bx,bY,56,15,1.5,1.5,"F");
      doc.setFontSize(6.5);doc.setFont("helvetica","normal");doc.setTextColor(120);doc.text(lbl.toUpperCase(),bx+3,bY+4.5);
      doc.setFontSize(11);doc.setFont("helvetica","bold");doc.setTextColor(0);doc.text(String(val),bx+3,bY+12);
    });
    let sy=M+33;const sW=PW-M*2;
    ...
    doc.setFontSize(6.5);doc.setFont("helvetica","bold");doc.setTextColor(255);
    let cx2=M+1;sC.forEach((h,i)=>{doc.text(h,i===0?cx2:cx2+sCW[i]-1,sy+3.8,{align:i===0?"left":"right"});cx2+=sCW[i];});
    sy+=5.5;doc.setTextColor(0);
    savedShelves.forEach((sh,i)=>{
      const vp=(sh.pls||[]).filter(p=>!p.invalid);const units=vp.reduce((s,pl)=>s+(pl.layers||1),0);
      const ov=sh.wtLimitI&&sh.wt>parseFloat(sh.wtLimitI);const o=sh.outer||{};
      ...
      [sh.name,sh.shape==="oval"?"Basket":"Tray",`${o.l||"?"}x${o.w||"?"}`,`${sh.tray?sh.tray.l:"?"}x${sh.tray?sh.tray.w:"?"}`,String(vp.length),String(units),sh.wt?sh.wt.toFixed(2):"–",sh.wtLimitI||"–",ov?"OVER":"OK"].forEach((v,vi)=>{
        ...
        doc.text(v,vi===0?cx2:cx2+sCW[vi]-1,sy+3.8,{align:vi===0?"left":"right",maxWidth:sCW[vi]-1});cx2+=sCW[vi];
      });
      doc.setDrawColor(230,230,230);doc.setLineWidth(0.1);doc.line(M,sy+5.5,M+sW,sy+5.5);sy+=5.5;
    });

    doc.addPage();
    doc.setFont("helvetica","bold");doc.setFontSize(13);doc.setTextColor(0);doc.text("Product Totals — All Shelves",M,M+5);
    [["SKUs",totalData.byProd.length],["Total Units",totalData.grandQty],["Total Weight",totalData.grandWt.toFixed(2)+" lbs"]].forEach(([lbl,val],i)=>{
      const bx=M+i*65,bY=M+12;doc.setFillColor(245,245,245);doc.roundedRect(bx,bY,60,15,1.5,1.5,"F");
      doc.setFontSize(6.5);doc.setFont("helvetica","normal");doc.setTextColor(120);doc.text(lbl.toUpperCase(),bx+3,bY+4.5);
      doc.setFontSize(11);doc.setFont("helvetica","bold");doc.setTextColor(0);doc.text(String(val),bx+3,bY+12);
    });
    let ty2=M+33;const tW2=PW-M*2;
    ...
    doc.setFontSize(6.5);doc.setFont("helvetica","bold");doc.setTextColor(255);
    cx2=M+1;tC2.forEach((h,i)=>{doc.text(h,i===0?cx2:cx2+tCW2[i]-1,ty2+3.8,{align:i===0?"left":"right"});cx2+=tCW2[i];});
    ...
    totalData.byProd.forEach((r,i)=>{
      ...
      [r.name,String(r.totalQty),r.unitWt?r.unitWt.toFixed(2):"–",r.totalWt?r.totalWt.toFixed(2):"–"].forEach((v,vi)=>{
        doc.text(v,vi===0?cx2:cx2+tCW2[vi]-1,ty2+3.8,{align:vi===0?"left":"right",maxWidth:tCW2[vi]-1});cx2+=tCW2[vi];
      });
      doc.setDrawColor(230,230,230);doc.setLineWidth(0.1);doc.line(M,ty2+5.5,M+tW2,ty2+5.5);ty2+=5.5;
    });
    ...
    cx2=M+tCW2[0]+tCW2[1];doc.text(String(totalData.grandQty),cx2-1,ty2+4,{align:"right"});
    cx2+=tCW2[2]+tCW2[3];doc.text(totalData.grandWt.toFixed(2),cx2-1,ty2+4,{align:"right"});
    ...
    <div style={{display:"flex",height:"100vh",background:"#e8e8e8",fontFamily:"system-ui,sans-serif",overflow:"hidden"}}>
      ...
      <div style={{width:296,background:"#f0f0f0",borderRight:"1px solid #ccc",display:"flex",flexDirection:"column",overflow:"hidden",flexShrink:0}}>
        ...
        <div style={{margin:"6px 6px 0",borderRadius:8,border:"1.5px solid #bbb",overflow:"hidden",flexShrink:0}}>
          ...
          {hdr("#4a5b7c","Capacity (footprint area)")}
          <div style={{padding:"8px 10px",background:"#fff"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
              <span style={{fontSize:10,color:"#666"}}>{Math.round(usedArea)} / {Math.round(trayArea)} in²</span>
              <span style={{fontSize:10,fontWeight:600,color:capColor}}>{capPct.toFixed(0)}%</span>
            </div>
            <div style={{height:6,background:"#e0e0e0",borderRadius:3,overflow:"hidden"}}>
              <div style={{height:"100%",width:capBarPct+"%",background:capColor,borderRadius:3,transition:"width 0.15s ease"}}/>
            </div>
            {capOver && (
              <div style={{fontSize:9.5,color:"#c0392b",marginTop:5}}>Entered quantities exceed the usable area — some items won't fit even before auto-arranging.</div>
            )}
          </div>
          ...
          <div style={{padding:"8px 10px",background:"#fff",borderBottom:activeWt>0?"1px solid #e0e0e0":"none"}}>
            <div style={{display:"flex",gap:6}}>
              <div style={{flex:1}}><div style={{fontSize:9,color:"#777",marginBottom:2,textAlign:"center"}}>All shelves</div><input value={globalWtI} onChange={e=>setGlobalWtI(e.target.value.replace(/[^0-9.]/g,""))} style={Object.assign({},inp,{textAlign:"center"})}/></div>
              <div style={{flex:1}}><div style={{fontSize:9,color:"#777",marginBottom:2,textAlign:"center"}}>{active.name}</div><input value={active.wtLimitI} onChange={e=>upd({wtLimitI:e.target.value.replace(/[^0-9.]/g,"")})} style={Object.assign({},inp,{textAlign:"center"})}/></div>
            </div>
          </div>
          {activeWt>0&&(<div style={{padding:"6px 10px",background:"#fafafa"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:10,color:"#666"}}>{active.name}</span><span style={{fontSize:10,fontWeight:600,color:wtOver?"#c0392b":wtPct>85?"#e67e22":"#111"}}>{activeWt.toFixed(2)}{wtLim?" / "+wtLim:""} lbs</span></div>
        ...
        {activeProds.length>0&&(<div style={{margin:"6px 6px 0",borderRadius:8,border:"1.5px solid #bbb",overflow:"hidden",flexShrink:0}}>
          {hdr("#444","Load Order")}
          <div style={{padding:"4px 6px",background:"#fff"}}>
            {activeProds.map((p,i)=>(
              <div key={p.id} draggable onDragStart={()=>setOrderDrag(p.id)} onDragOver={e=>{e.preventDefault();setOrderDragOver(p.id);}} onDragLeave={()=>setOrderDragOver(null)} onDrop={()=>handleOrderDrop(p.id)} onDragEnd={()=>{setOrderDrag(null);setOrderDragOver(null);}}
                ...
                <div style={{display:"flex",gap:2}}>{Array.from({length:(qtys[p.id]||{}).asst||0},(_,ai)=>(<div key={ai} style={{width:7,height:7,borderRadius:isRect(p)?1:"50%",background:"#fff",border:"1.5px solid "+getColor(p.id,ai),boxSizing:"border-box"}}/>))}</div>
              </div>
            ))}
          </div>
        </div>)}
        ...
        <div style={{margin:"6px 6px 0",borderRadius:8,border:"1.5px solid #bbb",overflow:"hidden",flexShrink:0}}>
          ...
          <div style={{padding:"6px 8px",background:"#fff"}}><input placeholder="Search..." value={search} onChange={e=>setSearch(e.target.value)} style={inp}/></div>
        </div>

        <div style={{flex:1,overflowY:"auto",background:"#f0f0f0",paddingBottom:6}}>
          {BRANDS.map(brand=>{
            const bc=BRAND_COLORS[brand];const bp=shown.filter(p=>p.brand===brand);if(!bp.length)return null;
            const bColl=collBrands.has(brand);const bkts={};
            bp.forEach(p=>{const b=p.bucket==="Small/Other"?"Standard Size":(p.bucket||"Standard Size");if(!bkts[b])bkts[b]=[];bkts[b].push(p);});
            return (
              <div key={brand} style={{margin:"6px 6px 0",borderRadius:8,border:"2px solid "+bc.border,overflow:"hidden"}}>
                ...
                {!bColl&&Object.keys(bkts).sort().map(bucket=>{
                  const bKey=brand+"|"+bucket;const bkColl=collBuckets.has(bKey);
                  return (
                    <div key={bucket} style={{margin:5,borderRadius:6,border:"1px solid "+bc.border+"88",overflow:"hidden",background:bc.light}}>
                      ...
                      {!bkColl&&(<div style={{padding:4}}>
                        {bkts[bucket].slice().sort((a,b)=>a.name.localeCompare(b.name)).map(p=>{
                          const q=qtys[p.id]||{asst:0,eachArr:[]};const on=q.asst>0;
                          return (
                            <div key={p.id} style={{marginBottom:3,borderRadius:6,background:on?"#e8f4fd":"#fff",border:"0.5px solid "+(on?"#90caf9":"#e0e0e0"),overflow:"hidden"}}>
                              <div style={{display:"flex",alignItems:"center",padding:"5px 6px",gap:4}}>
                                ...
                                <div style={{flex:1,minWidth:0}}>
                                  ...
                                  <div style={{fontSize:9,color:"#888"}}>{p.l}"×{p.w}" · {p.wt?p.wt.toFixed(2)+" lbs":"-"}</div>
                                </div>
                              ...
                              {on&&(<div style={{borderTop:"0.5px solid #e0e0e0"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{padding:"6px 10px",background:"#fff",borderBottom:"0.5px solid #e0e0e0",display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          {!showTotal&&validPls.length>0&&(<button onClick={saveShelf} style={{padding:"5px 10px",border:"0.5px solid "+(hasChanges?"#f0ad4e":isSaved?"#81c784":"#ccc"),borderRadius:6,background:hasChanges?"#fff8e1":isSaved?"#e8f5e9":"#f5f5f5",color:hasChanges?"#9a6700":isSaved?"#2e7d32":"#333",cursor:"pointer",fontSize:11,fontWeight:hasChanges?600:400}}>{hasChanges?"Update Shelf":isSaved?"Shelf Saved":"Save Shelf"}</button>)}
          ...
          <label style={{padding:"5px 10px",border:"0.5px solid #ccc",borderRadius:6,background:"#f5f5f5",color:"#333",cursor:"pointer",fontSize:11,fontWeight:500,display:"inline-flex",alignItems:"center"}}>Import Layout<input type="file" accept=".json" onChange={importJSON} style={{display:"none"}}/></label>
          <div style={{flex:1}}/>
          {!showTotal&&<span style={{fontSize:10,color:"#888"}}>{active.shape==="oval"?"Basket":"Tray"} · {outer.l}×{outer.w} outer</span>}
          ...
          {showTotal?(
            !savedShelves.length?(
              ...
              <div style={{width:"100%",maxWidth:820,display:"flex",flexDirection:"column",gap:16}}>
                ...
                {savedShelves.map(sh=>{
                  const vp=(sh.pls||[]).filter(p=>!p.invalid);const grp={};
                  ...
                  return (
                    <div key={sh.shelfId} style={{background:"#fff",borderRadius:8,border:"0.5px solid #e0e0e0",overflow:"hidden"}}>
                      <div style={{padding:"8px 12px",borderBottom:"0.5px solid #e0e0e0",background:"#f5f5f5",fontSize:11,fontWeight:600}}>
                        {sh.name}<span style={{fontSize:9,color:"#888",fontWeight:400}}> · {sh.shape==="oval"?"Basket":"Tray"} · {o.l||"?"}×{o.w||"?"} outer / {sh.tray?sh.tray.l:"?"}×{sh.tray?sh.tray.w:"?"} inner · {sh.savedAt}</span>
                      </div>
                      <div style={{padding:"20px 12px 24px",display:"flex",justifyContent:"center"}}><Thumb sh={sh} allProds={allProds} maxW={620} maxH={230}/></div>
                      <table style={{width:"100%",borderCollapse:"collapse"}}>
                        ...
                        <tbody>{Object.keys(grp).map((k,i)=>{const g=grp[k];const p=allProds.find(x=>x.id===g.pid);if(!p)return null;const col=(sh.colorMap||{})[k]||PALETTE[i%PALETTE.length];return(<tr key={k} style={{background:i%2?"#fafafa":"transparent"}}><td style={cell}><span style={{display:"inline-flex",alignItems:"center",gap:6}}><span style={{width:9,height:9,borderRadius:isRect(p)?2:"50%",background:"#fff",border:"2px solid "+col,boxSizing:"border-box",display:"inline-block"}}/>{p.name}</span></td><td style={Object.assign({},cell,{textAlign:"right"})}>{al(g.ai)}</td><td style={Object.assign({},cell,{textAlign:"right"})}>{g.c}</td><td style={Object.assign({},cell,{textAlign:"right"})}>{g.ly}</td><td style={Object.assign({},cell,{textAlign:"right",fontWeight:500})}>{g.c*g.ly}</td><td style={Object.assign({},cell,{textAlign:"right",color:"#888"})}>{p.wt?p.wt.toFixed(2):"-"}</td><td style={Object.assign({},cell,{textAlign:"right",fontWeight:500})}>{p.wt?(p.wt*g.c*g.ly).toFixed(2):"-"}</td></tr>);})}</tbody>
                      </table>
                    </div>
                  );
                })}
                <div style={{background:"#fff",borderRadius:8,border:"0.5px solid #e0e0e0",overflow:"hidden"}}>
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse"}}>
                    ...
                    <tbody>
                      {totalData.byProd.map((r,i)=>(<tr key={r.name} style={{background:i%2?"#fafafa":"transparent"}}><td style={cell}>{r.name}</td><td style={Object.assign({},cell,{textAlign:"right",fontWeight:500})}>{r.totalQty}</td><td style={Object.assign({},cell,{textAlign:"right",color:"#888"})}>{r.unitWt?r.unitWt.toFixed(2):"-"}</td><td style={Object.assign({},cell,{textAlign:"right",fontWeight:500})}>{r.totalWt?r.totalWt.toFixed(2):"-"}</td></tr>))}
                      <tr style={{background:"#f5f5f5",borderTop:"1px solid #ddd"}}><td style={Object.assign({},cell,{fontWeight:600})}>Grand total</td><td style={Object.assign({},cell,{textAlign:"right",fontWeight:600})}>{totalData.grandQty}</td><td style={cell}></td><td style={Object.assign({},cell,{textAlign:"right",fontWeight:600,color:gOver?"#c0392b":"#222"})}>{totalData.grandWt.toFixed(2)}{gLim?" / "+gLim:""}</td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
            ...
            <div style={{textAlign:"center",color:"#999",marginTop:60}}>
              ...
              <div style={{fontSize:11,marginTop:8,color:"#aaa"}}>Inner usable area: {tray.l}" × {tray.w}"</div>
            </div>
          ):(
            <>
              ...
              {legend.length>0&&(<div style={{width:"100%",maxWidth:540,background:"#fff",borderRadius:8,border:"0.5px solid #e0e0e0",padding:"8px 12px"}}>
                <div style={{fontSize:9,color:"#888",fontWeight:600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:6}}>Legend</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:"5px 16px"}}>{legend.map(o=>(<div key={o.key} style={{display:"flex",alignItems:"center",gap:6}}><span style={{width:11,height:11,borderRadius:isRect(o.prod)?2:"50%",background:"#fff",border:"2px solid "+o.color,boxSizing:"border-box",display:"inline-block"}}/><span style={{fontSize:10,color:"#444"}}>{o.prod.name} · <b>{al(o.ai)}</b></span></div>))}</div>
              </div>)}
              {summaryRows.length>0&&(<div style={{width:"100%",maxWidth:540,background:"#fff",borderRadius:8,border:"0.5px solid #e0e0e0",overflow:"hidden"}}>
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",padding:"8px 12px",borderBottom:"0.5px solid #e0e0e0",background:"#f5f5f5"}}>
                  {[["Columns",(active.layoutStats||{}).cols||"-"],["Positions",validPls.length],["Units",summaryRows.reduce((s,r)=>s+r.total,0)],["Weight",activeWt.toFixed(2)+" lbs"]].map(o=>(<div key={o[0]} style={{textAlign:"center"}}><div style={{fontSize:9,color:"#888",marginBottom:2}}>{o[0]}</div><div style={{fontSize:15,fontWeight:600,color:wtOver&&o[0]==="Weight"?"#c0392b":"#222"}}>{o[1]}</div></div>))}
                </div>
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  <thead><tr>{["Product","Asst.","Rows","Layers","Total","Unit wt","Total wt"].map(h=>(<th key={h} style={Object.assign({},hcell,{textAlign:h==="Product"?"left":"right"})}>{h}</th>))}</tr></thead>
                  <tbody>{summaryRows.map((r,i)=>(<tr key={r.key} style={{background:i%2?"#fafafa":"transparent"}}><td style={cell}><span style={{display:"inline-flex",alignItems:"center",gap:6}}><span style={{width:9,height:9,borderRadius:isRect(r.prod)?2:"50%",background:"#fff",border:"2px solid "+r.color,boxSizing:"border-box",display:"inline-block"}}/>{r.name}</span></td><td style={Object.assign({},cell,{textAlign:"right"})}>{al(r.ai)}</td><td style={Object.assign({},cell,{textAlign:"right"})}>{r.rows}</td><td style={Object.assign({},cell,{textAlign:"right"})}>{r.layers}</td><td style={Object.assign({},cell,{textAlign:"right",fontWeight:500})}>{r.total}</td><td style={Object.assign({},cell,{textAlign:"right",color:"#888"})}>{r.unitWt?r.unitWt.toFixed(2):"-"}</td><td style={Object.assign({},cell,{textAlign:"right",fontWeight:500})}>{r.totalWt?r.totalWt.toFixed(2):"-"}</td></tr>))}</tbody>
                </table>
              </div>)}
            </>
          )}
      ...
      {addOpen&&(<div onClick={()=>setAddOpen(false)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.35)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100}}>
        <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:10,padding:18,width:262}}>
          ...
          {[["name","Product name"],["cat","Category"],["l","Length (in)"],["w","Width (in)"],["wt","Weight (lbs)"]].map(o=>(<input key={o[0]} placeholder={o[1]} value={cf[o[0]]} onChange={e=>setCf(p=>Object.assign({},p,{[o[0]]:e.target.value}))} style={Object.assign({},inp,{marginBottom:6})}/>))}
          <select value={cf.brand} onChange={e=>setCf(p=>Object.assign({},p,{brand:e.target.value}))} style={Object.assign({},inp,{marginBottom:10})}>{BRANDS.map(b=>(<option key={b} value={b}>{b}</option>))}</select>
          <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
            ...
            <button onClick={()=>{if(!cf.name||!cf.l||!cf.w)return;setCustom(p=>p.concat([{id:"c_"+Date.now(),name:cf.name,cat:cf.cat||"Custom",brand:cf.brand||"Bath & Body",bucket:"Standard Size",l:parseFloat(cf.l),w:parseFloat(cf.w),wt:parseFloat(cf.wt)||0}]));setCf({name:"",cat:"Custom",brand:"Bath & Body",l:"",w:"",wt:""});setAddOpen(false);}} style={{padding:"5px 14px",border:"0.5px solid #ccc",borderRadius:6,background:"#f5f5f5",color:"#222",cursor:"pointer",fontSize:12,fontWeight:500}}>Add</button>
          </div>
        </div>
      </div>)}
    </div>
